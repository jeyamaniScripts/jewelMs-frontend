...
import { fetchRolePermissions, saveRolePermissions } from "@/redux/slices/permissionSlice";
...
export default function RolesPermissionsPage() {
  ...
  useEffect(() => {
    ...
    const source = roles.find((r) => r.roleKey === selectedRoleKey)?.permissions ?? {};
    const seeded = MENU_CONFIG.reduce((acc, item) => {
  ...
  const handleSave = async () => {
    const result = await dispatch(saveRolePermissions({ roleKey: selectedRoleKey, permissions: draft }));
    if (saveRolePermissions.fulfilled.match(result)) setSaved(true);
  };

  const selectedLabel = roles.find((r) => r.roleKey === selectedRoleKey)?.label ?? selectedRoleKey;
    ...
    <div>
      ...
      {isLoadingRoles ? (
        ...
        <>
          ...
          {saved && (
            <div className="mb-4">
              <Alert variant="success">Saved permissions for {selectedLabel}.</Alert>
            </div>
          )}
          ...
          <div className="mt-4 flex justify-end">
            <Button type="button" onClick={handleSave} isLoading={isSaving} fullWidth={false} className="px-6">
